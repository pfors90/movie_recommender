...
def import_vector_data(cursor, ids=None):
    # grab all existing movies and build a list of those missing vectors
    result = cursor.execute("SELECT id, vector FROM movies")
    missing_vectors = [m["id"] for m in result if m["vector"] is None]
    logger(f"import_vector_data() found {len(missing_vectors)} movies in the database with missing vectors", type='a')
    ...
    # sql query returns distinct (movie_id, keyword) pairs - keywords need to be build into a list
    ...
    for id, keywords in ids_keywords.items():
        ...
        # will need to use a custom tokenizer in vectorization process
    ...
    num_movies = vector_matrix.shape[0]
...
# unnecessary as TfidfVectorizer() returns normalized vectors already
